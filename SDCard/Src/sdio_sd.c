...
SD_Error SD_PowerON(void)
{
  ...
  {
	/*!< MMC card */
    /*!< Send CMD0 SDIO_SEND_OP_COND with Argument 0x80FF8000*/
    CardType = SDIO_MULTIMEDIA_CARD;	  
	while((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
	{	   										   				   
		/*!< SEND CMD1 Commands_CMD  */	  
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)
{
  ...
  /*<Eliminate the CPSM state machine>*/
  ...
  {
	  return SD_LOCK_UNLOCK_FAILED;   //The card lock
  }

  /* set the size of the card*/
  /* mainly to send the CMD16 command to set */
  /* SDSC card, you can set the size of the block */
  /* SDHC card, the size of the block is 512byte, not affected by CMD16*/
  
  if((BlockSize>0)&&(BlockSize<=2048)&&((BlockSize&(BlockSize-1))==0))
  {
	 blockSize = convert_from_bytes_to_power_of_two(BlockSize);	    	   
	/*< send CMD16+ set data length blksize, short response>*/	 
	...
	errorstatus = CmdResp1Error(SD_CMD_SET_BLOCKLEN);	//wait for R1 response   
	if(errorstatus != SD_OK)	
	{
		return errorstatus;	
	}
  ...
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t)blockSize << 4; // (uint32_t) 9 << 4;
  ...
  /*!< Polling mode */
  __ASM volatile("cpsid i"); /*!<Shutdown total interrupt (POLLING mode,
                              do not interrupt SDIO read and write operation!!! >*/  
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  /*<determine if you exceed the maximum buffer you receive>*/
  if(NumberOfBlocks * BlockSize > SD_MAX_DATA_LENGTH)
  {
		errorstatus = SD_INVALID_PARAMETER; 	// over, error message is returned
		return errorstatus;
  }
...
#if defined (SD_POLLING_MODE)  
  /*!< In case of multi block transfer, no need of stop transfer at all.*/
  /*!< Polling mode */
  __ASM volatile("cpsid i"); /*!<Shutdown total interrupt (POLLING mode,
                              do not interrupt SDIO read and write operation!!! >*/ 
  while(!(SDIO->STA &(SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DBCKEND | SDIO_FLAG_STBITERR)))
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize)
{
  ...
  SDIO->DCTRL = 0x0;  //the data control register is clear and the DMA is closed
  ...
  errorstatus = CmdResp1Error(SD_CMD_SEND_STATUS);	// wait for R1 response
  if(errorstatus!=SD_OK)
  {
	   return errorstatus;
  }
  ...
  while(((cardstatus&0x00000100) == 0 ) && (timeout > 0)) 	//check the READY_FOR_DATA bit to be placed
  {
    timeout--;
	 /*!<send CMD13 query card status >*/
	SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)RCA<<16;
  ...
  StopCondition = 0;/*!<a single block that does not need to send 
                     a stop transmission instruction>*/
  SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  SDIO_DataInitStructure.SDIO_DataLength = BlockSize;
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t)blockSize << 4; //(uint32_t) 9 << 4;
  SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToCard;
  ...
  /*!< In case of single data block transfer no need of stop command at all */
	...
	if(timeout==0)
	{
		SD_Init();	            // reinitializing SD card can solve the problem of writing dead machine
		return SD_DATA_TIMEOUT;	// timeout	 
	}
	timeout = SDIO_DATATIMEOUT;
	...
	SDIO_ClearFlag(SDIO_STATIC_FLAGS);	/*!< Clear all the static status flags*/
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    /*!< In case of multi data block transfer no need of stop command at all */
#if defined (SD_POLLING_MODE) 
	__ASM volatile("cpsid i"); /*!<Shutdown total interrupt (POLLING mode,
								do not interrupt SDIO read and write operation!!! >*/
	while (!(SDIO->STA & (SDIO_FLAG_DBCKEND | SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)))
    ...
    SDIO_ClearFlag(SDIO_STATIC_FLAGS);	/*!< Clear all the static status flags*/+
	__ASM volatile("cpsie i");/*!<Open total interrupt>*/ 
...
#endif		 
	SDIO_ClearFlag(SDIO_STATIC_FLAGS);	 // clear all marks
	errorstatus = IsCardProgramming(&cardstate);
	while((errorstatus==SD_OK) && ((cardstate==SD_CARD_PROGRAMMING) || (cardstate==SD_CARD_RECEIVING)))
	{
		errorstatus = IsCardProgramming(&cardstate);
...
{
	if(SDIO_GetITStatus(SDIO_IT_DATAEND) != RESET) //receive complete interruption
	{	 
		if (StopCondition==1)
		{
			/*!<send CMD12 and end the transmission>*/	   
			SDIO_CmdInitStructure.SDIO_Argument = 0;
		...
		SDIO_ClearITPendingBit(SDIO_IT_DATAEND);   //clear off the interrupt mark
		SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
					  SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_TXUNDERR |
					  SDIO_IT_RXOVERR | SDIO_IT_STBITERR, DISABLE); // close related interrupts
		TransferEnd = 1;
		return(TransferError);
	}
	if(SDIO_GetITStatus(SDIO_IT_DCRCFAIL) != RESET)  //data CRC error
	{
		SDIO_ClearITPendingBit(SDIO_IT_DCRCFAIL);  // remove interrupt flags
		SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
					  SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_TXUNDERR |
 					  SDIO_IT_RXOVERR | SDIO_IT_STBITERR, DISABLE);  // close related interrupts
	...
	if(SDIO_GetITStatus(SDIO_IT_DTIMEOUT) != RESET)  // data timeout error
	{
		SDIO_ClearITPendingBit(SDIO_IT_DTIMEOUT);// remove interrupt flags
		SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
					  SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_TXUNDERR |
					  SDIO_IT_RXOVERR | SDIO_IT_STBITERR, DISABLE);// close related interrupts
	...
	if(SDIO_GetITStatus(SDIO_IT_RXOVERR) != RESET) //the overflow error on FIFO
	{
		SDIO_ClearITPendingBit(SDIO_IT_RXOVERR);// remove interrupt flags
		SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
					  SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_TXUNDERR |
					  SDIO_IT_RXOVERR | SDIO_IT_STBITERR, DISABLE);// close related interrupts 
	...
	if(SDIO_GetITStatus(SDIO_IT_TXUNDERR) != RESET) //a FIFO error
	{
		SDIO_ClearITPendingBit(SDIO_IT_TXUNDERR);// remove interrupt flags
		SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
					  SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_TXUNDERR |
					  SDIO_IT_RXOVERR | SDIO_IT_STBITERR, DISABLE);// close related interrupts  
	...
	if(SDIO_GetITStatus(SDIO_IT_STBITERR) != RESET) //start bit error
	{
		SDIO_ClearITPendingBit(SDIO_IT_STBITERR);// remove interrupt flags
		SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
					  SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_TXUNDERR |
					  SDIO_IT_RXOVERR | SDIO_IT_STBITERR, DISABLE);// close related interrupts 
